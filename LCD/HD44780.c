...
//
//-------------------------------------------------------------------------------------------------
void LCD_GoTo2(unsigned char x, unsigned char y)
{
LCD_WriteCommand(HD44780_DDRAM_SET | (x + (0x40 * y)));
}
void LCD_GoTo4(unsigned char x, unsigned char y)
{
	if (y > 1)
	{
		y -= 2;
...
//-------------------------------------------------------------------------------------------------
//
// Funkcja konwertujπca int na string.
//
//-------------------------------------------------------------------------------------------------
char *itoa(int i)
{
...
//-------------------------------------------------------------------------------------------------
//
// .Funkcja wyúwietlenia liczby na wyswietlaczu.
//
//-------------------------------------------------------------------------------------------------
void LCD_WriteInt(int temp_int,uint8_t param)
{
	if (param==1)								// funkcja umoøliwia poprzedzenie "0" wyúwietlanej liczby, gdy jest ona mniejsza od zera
	{	if (temp_int<10)
			LCD_WriteText("0");
	}
	LCD_WriteText(itoa(temp_int));
}
//-------------------------------------------------------------------------------------------------
//
// .Funkcja wyúwietlenia liczby na wyswietlaczu.w postaci binarnej
//
//-------------------------------------------------------------------------------------------------
void LCD_TempB(int tmp)
{
	for (int a=7;a>=0;a--)
	{
		if (tmp & (1<<a))
